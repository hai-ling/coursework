...
while true
    ...
    % >>>>>>>>>> Assignment: use manhattan distance(cityblock)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for j = 1:n_cluster
        %NaN array
        sum_distances = NaN(size(membership));
        % loop thru each datapoint
        for i = 1:n_sample
            if membership(i) == j  %for each datapoint in cluster, sum manhattan distance with all of its cluster neighbors 
                total = 0;
                for x = 1:n_sample
                    if membership(x) == membership(i) 
                        total = total + pdist2(X(x, :), X(i, :), 'cityblock');
                    end
                sum_distances(i) = total;
                end
            end
        end
        %choose datapoint with smallest sum
        [~, I] = min(sum_distances, [], 'omitnan');
        centres(j, :) = X(I,:);
    end
